...
// APIKeyResourceScope defines which resources API keys are allowed to access.
type APIKeyResourceScope struct {
	AllowedResources []Resource
}

// Config holds all RBAC configuration. Zero defaults — everything must be
// provided by the caller (or by a preset).
type Config struct {
	...
	APIKeyScope           *APIKeyResourceScope // nil = deny all API key access
}
...
func (c *Config) Validate() error {
	...
	// Validate Capabilities references
	...
	// Validate PermissionToActionMap references and uniqueness
	...
	// Validate APIKeyScope references
